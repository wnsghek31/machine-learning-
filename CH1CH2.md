...
### Bias 
Bias란 학습데이터를 충분히 표현할 수 없기 때문에 발생. 충분한 데이터가 있지만 , 데이터 안에 있는 데이터 간의 상관관계를 충분히 풀어내지 못할떄 발생한다.
결과적으로 모델은 지속적으로 데이터를 잘못 표현하게 되므로써 낮은 정답률을 보인다.
...
에러는 bias 와 variance의 합.
관측에서 필연적으로 예측값과 참값 사이의 에러가 존재할수밖에없다. 이것을 노이즈. 
variance는 prediction들의 중앙값과 각 prediction의 차이 제곱들의 합

머신러닝에 에러는 두가지로 분류할수있는데, 편향(Bias)과 분산(Variance)입니다. 이 두 개념은 서로 다르게 움직이는데, 종종 Bias를 해결하면 , Variance가 올라가고 반대도.. 즉 TradeOff 관계이다.
...
모델의 복잡도는 훈련 셋에답긴 데이터의 다양성과 관련이 깊은데, 데이터 셋에 다양한 데이터 포인트가 많을수록
...
supervised learning의 아주 간단한 방법인 선형회귀. 다른 통계방법에 비해 간단하나 해석력이 뛰어나 여전히 널리쓰이고 있고 다른 방법들의 기초가 되는 지식이다
...
KNN은 Mark와 Kate를 잘분류하고 , SVM은 BOb과 Sue를 잘 분ㄹ하는 장점을 가지고 있끼에 둘을 결합하여 높은 정확도의 모델을 만들었다.
...
[그림3]
원래 특성에 다시 투영하면이 선형 SVM 모델은 더이상 선형이 아니다.
